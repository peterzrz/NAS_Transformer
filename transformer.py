...
Original file is located at
    https://colab.research.google.com/drive/1UpquXdaS7jjIMgsm-Y-19ZD5gReyxix-
...
class Encoder(tf.keras.layers.Layer):
  def __init__(self, *, num_layers, d_model, num_heads,
    ...
    if(len(activation) == 0):
        activation = ['relu' for i in range(num_layers)]
    self.d_model = d_model
...
class Decoder(tf.keras.layers.Layer):
  def __init__(self, *, num_layers, d_model, num_heads, dff, vocab_size,
    ...
    self.dropout = tf.keras.layers.Dropout(dropout_rate)
    self.dec_layers = [
        DecoderLayer(d_model=d_model, num_heads=num_heads[i],
                     dff=dff[i], dropout_rate=dropout_rate, activation = activation[i])
        for i in range(num_layers)]
